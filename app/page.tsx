...
function PerfumeCard({ p }: { p: typeof PERFUMES[0] }) {
  ...
  return (
    <div
      ...
      style={expanded ? { borderColor: G_BORDER } : {}}
    >
      {/* Clickable header */}
      <button
      ...
      >
        ...
        <div className="flex items-start justify-between gap-3 mb-5">
          ...
          <div className="shrink-0 flex flex-col items-end gap-1.5 pt-0.5">
            ...
            <span
              ...
              style={{ color: G_DIM }}
            >
              {p.layerOrder.toUpperCase()} LAYER
            </span>
          </div>
        </div>

        {/* Attribute bars — cleaner spacing */}
        ...
        {/* Top note preview — restrained */}
        <div className="flex flex-wrap gap-1 mb-4">
          {p.notes.top.slice(0, 3).map(n => (
            <span key={n} style={{ fontSize: '10px', padding: '2px 8px', background: 'hsl(190 70% 60% / 0.07)', color: 'hsl(190 70% 65%)', border: '1px solid hsl(190 70% 60% / 0.2)', borderRadius: '999px' }}>{n}</span>
          ))}
          {(p.notes.heart.length + p.notes.base.length) > 0 && (
            <span style={{ fontSize: '10px', padding: '2px 8px', background: 'hsl(220 14% 14%)', color: 'hsl(220 10% 48%)', border: '1px solid hsl(220 14% 20%)', borderRadius: '999px' }}>
        ...
        {/* Expand hint */}
        <div className="flex items-center gap-1.5" style={{ color: expanded ? G : G_DIM }}>
          {expanded
      ...
      {expanded && (
        <div
          className="border-t px-6 pb-6 pt-5 space-y-6"
          style={{ borderColor: 'hsl(220 14% 16%)' }}
        ...
        >
          ...
          <div>
            ...
            <div className="space-y-4">
              {[
                { icon: Wind,     color: 'hsl(190 70% 60%)', label: 'Top Notes',   timing: '0–30 min',    notes: p.notes.top },
                { icon: Sparkles, color: G,                   label: 'Heart Notes', timing: '30 min–3 hr', notes: p.notes.heart },
                { icon: Flame,    color: 'hsl(30 80% 60%)',   label: 'Base Notes',  timing: '3 hr+',       notes: p.notes.base },
                ...
                <div key={label}>
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[10px] font-mono text-muted-foreground ml-auto">{timing}</span>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {notes.map(n => (
                      <span key={n} style={{ fontSize: '10px', padding: '2px 8px', background: `${color.slice(0,-1)} / 0.08)`, color, border: `1px solid ${color.slice(0,-1)} / 0.22)`, borderRadius: '999px' }}>{n}</span>
                    ))}
                  </div>
                </div>
              ))}
...
function StackCard({ stack }: { stack: typeof STACKS[0] }) {
  ...
  return (
    <div className="panel-glow overflow-hidden transition-all duration-200">
      <div className="p-7">
        ...
        <div className="flex items-start justify-between gap-4 mb-5">
          ...
          <div className="text-right shrink-0">
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5 font-mono">MATCH</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-5">
          {perfumes.map((p, i) => (
            <div key={p.id} className="flex items-center gap-2">
              {i > 0 && (
                <span className="text-xs font-mono" style={{ color: G_DIM }}>+</span>
              )}
        ...
        {/* Season */}
        <p className="text-[10px] font-mono tracking-wider text-muted-foreground mb-3">{stack.season.toUpperCase()}</p>
      ...
      {expanded && (
        <div
          className="border-t px-7 pb-7 pt-5"
          style={{ borderColor: 'hsl(220 14% 16%)', background: 'hsl(220 16% 8%)' }}
        >
        ...
        <div className="space-y-5">
          <div className="flex items-center gap-2.5">
            ...
            <p className="text-sm text-muted-foreground">
              Best stacks for <strong className="text-foreground">{occasion?.label}</strong>
            </p>
          </div>
  ...
  return (
    <div>
      ...
      {suggestions.length > 0 ? (
        <div className="space-y-5">
          <p className="text-sm text-muted-foreground">
            Stacks you can build <strong className="text-foreground">right now</strong> from bottles you own:
          </p>
...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <button
      ...
      className="w-full text-left panel px-6 py-5 transition-colors"
      style={open ? { borderColor: G_BORDER } : {}}
    >
      <div className="flex items-center justify-between gap-4">
        <span className="text-sm font-medium text-foreground">{q}</span>
        {open
          ? <Minus size={13} className="shrink-0" style={{ color: G }} strokeWidth={2.5} />
          : <Plus  size={13} className="shrink-0 text-muted-foreground" strokeWidth={2} />}
      </div>
      {open && (
        <p className="text-sm text-muted-foreground leading-relaxed mt-4 pt-4 border-t" style={{ borderColor: 'hsl(220 14% 16%)' }}>
          {a}
        </p>
      )}
    </button>
...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main>
        ...
        <section className="hero-bg pt-40 pb-32 px-6 relative overflow-hidden">
          ...
          <div className="pointer-events-none absolute inset-0" style={{
            backgroundImage: 'linear-gradient(hsl(220 14% 15% / 0.3) 1px, transparent 1px), linear-gradient(90deg, hsl(220 14% 15% / 0.3) 1px, transparent 1px)',
            backgroundSize: '52px 52px',
            maskImage: 'radial-gradient(ellipse 80% 60% at 50% 50%, black 0%, transparent 100%)'
          }} />
          {/* Gold scanline */}
          <div className="pointer-events-none absolute inset-x-0 top-0 h-px opacity-40" style={{ background: `linear-gradient(90deg, transparent, ${G}, transparent)` }} />
          ...
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-20 items-center relative z-10">
            <div>
              ...
              <div className="flex items-center gap-2 mb-8">
                ...
                <div className="chip" style={{ background: 'hsl(142 70% 55% / 0.1)', color: 'hsl(142 70% 55%)', borderColor: 'hsl(142 70% 55% / 0.25)' }}>
                  <span className="inline-block w-[5px] h-[5px] rounded-full mr-1.5" style={{ background: 'hsl(142 70% 55%)' }} />
                  Live
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-start gap-3 mb-14">
                ...
                <Link
                  href="#stack"
                  className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors h-11 px-2 group"
                >
              ...
              <div className="flex items-center gap-5">
                ...
                <div className="h-5 w-px" style={{ background: 'hsl(220 14% 20%)' }} />
                <div>
            ...
            <div className="relative hidden md:block">
              <div
                className="aspect-[4/5] w-full max-w-sm ml-auto overflow-hidden rounded-2xl"
                style={{ boxShadow: '0 32px 80px hsl(0 0% 0% / 0.6)' }}
              >
                <img src={PHOTOS.hero} alt="Fragrance collection" className="w-full h-full object-cover brightness-70" />
              </div>
        ...
        {/* ── TRUST BAR ── */}
        <section className="py-12 px-6 border-y border-border" style={{ background: 'hsl(220 16% 8%)' }}>
          <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {[
              ...
              <div key={label}>
                <p className="text-lg font-medium serif mb-1" style={{ color: G }}>{stat}</p>
        ...
        <section id="stack" className="py-28 px-6">
          <div className="max-w-5xl mx-auto">
            ...
            {/* Tab switcher — cleaner pill design */}
            <div className="flex justify-center mb-12">
              <div
                className="flex items-center gap-1 p-1 rounded-2xl"
                style={{ background: 'hsl(220 16% 8%)', border: '1px solid hsl(220 14% 14%)' }}
              >
                {([
                  ...
                  <button
                    ...
                    style={
                      activeTab === id
                        ? { background: G_BG, color: G, border: `1px solid ${G_BORDER}` }
        ...
        <section id="tips" className="py-28 px-6">
          <div className="max-w-5xl mx-auto">
              ...
              {LAYERING_TIPS.map(({ icon: Icon, title, body }, i) => (
                <div key={title} className="panel p-7 flex flex-col gap-5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-3xl font-light serif" style={{ color: 'hsl(220 14% 18%)' }}>
                      {String(i + 1).padStart(2, '0')}
                    </span>
                  </div>
            ...
            {/* Pro tip callout */}
            <div
              className="mt-8 rounded-2xl p-8 grid md:grid-cols-2 gap-10 items-center"
              style={{ background: G_BG, border: `1px solid ${G_BORDER}` }}
            >
              <div>
          ...
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-20 items-center">
            <div>
              ...
              <h2 className="text-4xl md:text-5xl font-normal leading-tight mb-6 serif">
                ...
                <em className="italic" style={{ color: G }}>unforgettable scent.</em>
              </h2>
              ...
              {[
                ...
                <div key={term} className="panel p-5 flex gap-4">
                  <div
                    ...
                    style={{ background: `hsl(${color.match(/\d+ \d+% \d+%/)?.[0]} / 0.1)` }}
                  >
              ...
              {[
                ...
                <div key={step} className="panel p-7 flex flex-col gap-5">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-3xl font-light serif" style={{ color: 'hsl(220 14% 18%)' }}>{step}</span>
                  </div>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid md:grid-cols-3 gap-5">
              {[
                ...
                <div key={name} className="panel p-7 flex flex-col gap-5">
                  <div className="flex gap-0.5" style={{ color: G }}>
                    {[1,2,3,4,5].map(s => <Star key={s} size={11} fill="currentColor" strokeWidth={0} />)}
                  </div>
                  <p className="text-sm text-foreground leading-relaxed flex-1">&ldquo;{quote}&rdquo;</p>
                  <div className="flex items-center gap-3 pt-4 border-t" style={{ borderColor: 'hsl(220 14% 14%)' }}>
                ...
                <div key={plan.name} className={`rounded-2xl p-8 flex flex-col ${plan.highlighted ? 'panel-glow' : 'panel'}`}>
                  ...
                  <ul className="space-y-3.5 mb-9 flex-1">
                    {plan.features.map(f => (
                      <li key={f} className="flex items-start gap-2.5 text-sm text-muted-foreground">
                        <Check size={12} className="mt-1 shrink-0" style={{ color: G }} strokeWidth={2.5} />
                        {f}
                      </li>
                    ))}
                  </ul>
                  <Link href={plan.href}>
                    {plan.highlighted
                      ...
                      : <button className="w-full py-3 text-xs font-semibold rounded-full border border-border hover:border-[hsl(42_85%_68%_/_0.35)] transition-colors">{plan.cta}</button>
                    }
                  </Link>
                </div>
              ))}
        ...
        <section className="py-8 px-6 pb-16">
          <div className="max-w-4xl mx-auto">
            <div className="relative rounded-2xl overflow-hidden">
              <img src={PHOTOS.ctaBg} alt="" className="w-full h-72 object-cover" style={{ filter: 'brightness(0.35)' }} />
              <div
                className="absolute inset-0 flex flex-col items-center justify-end pb-14 px-8 text-center"
                style={{ background: 'linear-gradient(to top, hsl(220 18% 6% / 0.98), hsl(220 18% 6% / 0.45) 55%, transparent)' }}
              >
                ...
                <p className="text-white/55 text-sm mb-9 max-w-md leading-relaxed">
      ...
      {/* FOOTER */}
      <footer className="border-t border-border py-14 px-6" style={{ background: 'hsl(220 18% 5%)' }}>
        <div className="max-w-6xl mx-auto">
          ...
          <p className="text-xs text-muted-foreground mt-5">&copy; {new Date().getFullYear()} ScentStack. All rights reserved.</p>
        </div>
      </footer>
