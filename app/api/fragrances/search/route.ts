...
// Build a Fragrantica search URL and scrape the first image result
// Fragrantica CDN images follow the pattern:
// https://fimgs.net/mdimg/perfume/375x500.{id}.jpg
// We derive the ID by searching their site and pulling the first result image.
async function getFragranticaImage(name: string, brand: string): Promise<string | null> {
  try {
    ...
    // Extract first perfume image from search results
    // Pattern: <img src="https://fimgs.net/mdimg/perfume/375x500.XXXXX.jpg"
...
// Known Fragrantica image IDs for popular fragrances (avoids scraping for common ones)
...
export async function GET(request: NextRequest) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const results = (raw.data || raw.results || raw || []).map((f: any) => ({
          ...
          image_url: f.image || f.image_url || KNOWN_IMAGES[f.id] || null,
  ...
  // ── Demo data with real Fragrantica bottle images ───────────────────────────
  ...
  const results = matched.slice(0, 20).map(f => ({
    ...f,
    image_url: KNOWN_IMAGES[f.id] || null,
  }))

  // If nothing matched in demo, try a Fragrantica image scrape for unknown fragrances
  if (results.length === 0) {
