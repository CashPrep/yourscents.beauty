...
export default function DiscoveryFeed({ wardrobeIds }: { wardrobeIds: string[] }) {
  const unowned = TRENDING.filter(f => !wardrobeIds.includes(f.id))
          ...
          <div key={frag.id} className="bg-card border border-border rounded-2xl p-3 flex flex-col gap-2 hover:shadow-md transition-shadow">
            <div className="relative">
              <div className="w-full aspect-square rounded-xl bg-muted/30 overflow-hidden border border-border flex items-center justify-center">
                ...
                <img src={frag.image} alt={frag.name} className="w-full h-full object-contain p-2"
                  onError={(e) => { (e.target as HTMLImageElement).style.display='none' }} />
              </div>
              <span className="absolute top-1.5 left-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                style={{ background: ROSE, color: '#fff' }}>#{frag.rank}</span>
            </div>
            <div>
