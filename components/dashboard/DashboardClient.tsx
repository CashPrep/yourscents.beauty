...
import {
  ...
  Dna, Share2, Zap, Brain, CloudSun, ExternalLink, Lock, ArrowRight,
  PartyPopper, CreditCard, Heart, Trophy,
  type LucideIcon,
} from 'lucide-react'
...
// ↑ Free limit raised from 3 → 5 to reduce top-of-funnel friction
const FREE_LIMIT = 5
...
async function startCheckout(plan: 'pro'|'collector', setLoading: (v:boolean)=>void, setError: (v:string)=>void) {
  ...
  try {
    const res = await fetch('/api/checkout', { method:'POST', headers:{'Content-Type':'application/json'}, credentials:'include', body:JSON.stringify({plan}) })
    if (!res.ok) { const b = await res.json().catch(()=>({})); throw new Error((b as any).error||`Checkout failed (${res.status})`) }
...
async function openBillingPortal(setLoading: (v:boolean)=>void, setError: (v:string)=>void) {
  setLoading(true); setError('')
  try {
    const res = await fetch('/api/billing-portal', { method:'POST', credentials:'include' })
    if (!res.ok) { const b = await res.json().catch(()=>({})); throw new Error((b as any).error||'Could not open billing portal') }
    const { url } = await res.json() as { url:string }
...
function UpgradedBanner({ plan, onDismiss }: { plan: string; onDismiss: () => void }) {
  return (
    <div className="rounded-2xl p-5 flex items-center justify-between gap-4 mb-6"
      style={{ background:`linear-gradient(135deg,hsl(8 56% 76%/0.14),hsl(8 56% 76%/0.24))`, border:`1.5px solid ${ROSE_BORDER}`, boxShadow:`0 4px 24px hsl(8 56% 76%/0.14)` }}>
      <div className="flex items-center gap-4">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background:ROSE_BG, border:`1px solid ${ROSE_BORDER}` }}>
          <PartyPopper className="h-5 w-5" style={{ color:ROSE_DEEP }} />
        </div>
        <div>
          <p className="text-sm font-semibold" style={{ color:FOREGROUND }}>Welcome to {plan.charAt(0).toUpperCase()+plan.slice(1)}! 🌸</p>
          <p className="text-xs mt-0.5" style={{ color:MUTED }}>Your wardrobe is now unlimited. Start adding more fragrances!</p>
        </div>
      </div>
      <button onClick={onDismiss} className="text-xs shrink-0 px-3 py-1.5 rounded-full" style={{ color:MUTED, border:`1px solid ${ROSE_BORDER}` }}>Dismiss</button>
    </div>
  )
}
...
function FreePlanGate() {
  ...
  return (
    <div className="rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
      style={{ background:`linear-gradient(135deg,hsl(8 56% 76%/0.10),hsl(8 56% 76%/0.18))`, border:`1.5px solid ${ROSE_BORDER}`, boxShadow:`0 4px 24px hsl(8 56% 76%/0.12)` }}>
      <div className="flex items-start gap-4">
        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ background:ROSE_BG, border:`1px solid ${ROSE_BORDER}` }}>
          <Lock className="h-4 w-4" style={{ color:ROSE_DEEP }} />
        </div>
        <div>
          <p className="text-sm font-semibold mb-1" style={{ color:FOREGROUND }}>You&apos;ve reached the {FREE_LIMIT}-fragrance free limit</p>
          <p className="text-xs leading-relaxed" style={{ color:MUTED }}>
            Free plan includes up to {FREE_LIMIT} fragrances. Upgrade to Pro for an unlimited
            wardrobe, full stack scoring, advanced analytics, and shareable scent cards.
          </p>
          {error && <p className="text-xs mt-2 text-destructive">{error}</p>}
        </div>
      </div>
      <button onClick={() => startCheckout('pro', setLoading, setError)} disabled={loading}
...
function ScentCardPanel({ userId, copied, onCopy }: { userId: string; copied: boolean; onCopy: () => void }) {
  ...
  return (
    <div className="rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6"
      style={{ background:'hsl(0 0% 100%)', border:`1px solid ${ROSE_BORDER}`, boxShadow:`0 4px 20px hsl(8 56% 76%/0.10)` }}>
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-[11px] font-mono truncate max-w-[220px]" style={{ color:MUTED }}>{host}/u/{shortId}…</p>
        </div>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        <a href={`/u/${userId}`} target="_blank" rel="noopener noreferrer"
        ...
        <button onClick={onCopy} className="btn-gold flex items-center gap-1.5 text-xs px-4 py-2">
          <Share2 className="h-3 w-3" />{copied ? 'Copied! 🌸' : 'Copy Link'}
        </button>
      </div>
    </div>
  )
...
export default function DashboardClient({ user, wardrobe: initialWardrobe, profile, justUpgraded = false }: Props) {
  ...
  const handleFragranceAdded = (item: WardrobeItem) => { setWardrobe(prev => [item, ...prev]); setShowAddModal(false); setAddBlocked(false) }
  const handleAddClick       = () => { if (atLimit) { setAddBlocked(true); return }; setShowAddModal(true) }
  const handleRemove         = async (id: string) => {
    await fetch('/api/wardrobe', { method:'DELETE', headers:{'Content-Type':'application/json'}, credentials:'include', body:JSON.stringify({id}) })
  ...
  const displayWardrobe = [...wardrobe]
    .filter(item => accordFilter === 'all' || (item.accords||[]).some(a => a.toLowerCase().includes(accordFilter)))
    .sort((a,b) => {
  ...
  return (
    <div className="min-h-screen" style={{ background:CREAM, backgroundImage:'radial-gradient(ellipse 70% 40% at 50% -5%,hsl(8 56% 76%/0.12) 0%,transparent 65%),radial-gradient(ellipse 50% 30% at 100% 100%,hsl(13 48% 65%/0.08) 0%,transparent 60%)' }}>
      <header className="border-b sticky top-0 z-40" style={{ background:'hsl(18 60% 98%/0.88)', backdropFilter:'blur(18px)', borderColor:'hsl(10 30% 88%)' }}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Link href="/">
              <Image src="/logo.png" alt="Your Scents" width={120} height={44} className="h-10 w-auto object-contain" />
            </Link>
            <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={planStyle}>{planLabel}</span>
          </div>
          <div className="flex items-center gap-2">
            {isFree ? (
              <button onClick={() => startCheckout('pro',setUpgradeLoading,setUpgradeError)} disabled={upgradeLoading}
                className="btn-gold flex items-center gap-1.5 text-xs px-3 py-1.5">
                {upgradeLoading ? 'Opening…' : '✨ Upgrade'}
              </button>
            ) : (
              <button onClick={() => openBillingPortal(setPortalLoading,setPortalError)} disabled={portalLoading}
                className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full"
                style={{ background:ROSE_BG, color:ROSE_DEEP, border:`1px solid ${ROSE_BORDER}` }}>
                <CreditCard className="h-3 w-3" />{portalLoading ? 'Opening…' : 'Manage Plan'}
              </button>
            )}
            <button onClick={handleShare} className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full"
              style={{ background:ROSE_BG, color:ROSE_DEEP, border:`1px solid ${ROSE_BORDER}` }}>
              <Share2 className="h-3 w-3" />{copied ? 'Copied! 🌸' : 'My Scent Card'}
            </button>
        ...
        {(upgradeError||portalError) && <p className="text-xs text-center text-destructive pb-2">{upgradeError||portalError}</p>}
      </header>

      <div className="max-w-6xl mx-auto px-4 py-6">
        {showUpgradedBanner && <UpgradedBanner plan={plan} onDismiss={() => setShowUpgradedBanner(false)} />}

        <StreakTracker />

        {wardrobe.length > 0 && <DailyPulse wardrobe={wardrobe} />}

        {wardrobe.length > 0 && <ScentCardPanel userId={user.id} copied={copied} onCopy={handleShare} />}

        {wardrobe.length > 0 && <div className="mb-6"><ScentOfTheDay wardrobe={wardrobe} /></div>}
        ...
        {atLimit && <div className="mb-6"><FreePlanGate /></div>}
        ...
        <div className="flex gap-1 mb-6 overflow-x-auto pb-1">
          {tabs.map(({ id, label, icon:Icon, badge, isNew }) => (
            <button key={id} onClick={() => setActiveTab(id)}
              ...
              style={ activeTab===id ? { background:ROSE_BG, color:ROSE_DEEP, border:`1px solid ${ROSE_BORDER}` } : { color:MUTED, border:'1px solid transparent' } }>
              <Icon className="h-3.5 w-3.5" />
              {label}
              {badge!==undefined && badge>0 && <span className="text-[10px] px-1.5 py-0.5 rounded-full" style={{ background:ROSE_BG, color:ROSE_DEEP }}>{badge}</span>}
              {isNew && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background:ROSE, color:'#fff' }}>NEW</span>}
            </button>
          ))}
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
              <div className="flex gap-2 flex-wrap">
                {ACCORD_FILTERS.map(f => (
                  <button key={f} onClick={() => setAccordFilter(f)}
                    className="text-[11px] px-3 py-1.5 rounded-full capitalize transition-colors"
                    style={ accordFilter===f ? { background:ROSE_BG, color:ROSE_DEEP, border:`1px solid ${ROSE_BORDER}` } : { background:'hsl(18 40% 93%)', color:MUTED, border:'1px solid hsl(10 25% 86%)' } }>
                    {f}
                  </button>
                ))}
              </div>
              <div className="flex items-center gap-2">
                <select value={sort} onChange={e => setSort(e.target.value)}
                  className="text-[12px] bg-card border border-border rounded-lg px-3 py-1.5" style={{ color:FOREGROUND }}>
                  {SORT_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
                <button onClick={handleAddClick}
                  className="flex items-center gap-1.5 text-xs px-4 py-2 rounded-full font-semibold transition-all"
                  style={ atLimit ? { background:ROSE_BG, color:ROSE_DEEP, border:`1px solid ${ROSE_BORDER}` } : undefined }>
                  {atLimit ? <><Lock className="h-3 w-3" /> Upgrade</> : <><Plus className="h-3.5 w-3.5" /> Add</>}
                </button>
              </div>
            </div>

            {addBlocked && atLimit && <div className="mb-5"><FreePlanGate /></div>}

            {displayWardrobe.length === 0 && (
              <div className="text-center py-20 rounded-2xl border border-dashed" style={{ borderColor:ROSE_BORDER }}>
                ...
                <button onClick={() => setShowAddModal(true)} className="btn-gold text-sm px-6 py-2.5">Add Fragrance ✨</button>
              </div>
            )}
      ...
      {showAddModal && (
        <AddFragranceModal onClose={() => setShowAddModal(false)} onAdd={handleFragranceAdded} wardrobe={wardrobe} />
      )}
    </div>
  )
}
