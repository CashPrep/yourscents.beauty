...
import { Heart, Trash2, ShoppingBag, Plus, ExternalLink } from 'lucide-react'
        ...
        >
          <div className="grid sm:grid-cols-2 gap-2">
            <input
              ...
              className="text-sm px-3 py-2 rounded-xl border border-border bg-white w-full"
            />
            <input
              value={brand}
          ...
          <div className="flex items-center gap-2">
            ...
            {(['high','medium','low'] as const).map(p => (
              <button
                ...
                style={{
                  background: priority === p ? PRIORITY_LABELS[p].color : 'hsl(0 0% 100% / 0.7)',
                  color:      priority === p ? '#fff' : ROSE_TEXT,
                }}
              >{PRIORITY_LABELS[p].label}</button>
            ))}
            <button
              onClick={handleAdd}
              className="ml-auto text-xs font-bold px-4 py-1.5 rounded-full"
              style={{ background: ROSE, color: '#fff' }}
            >
              Save
            </button>
        ...
        <div className="flex gap-1.5">
          {(['all','high','medium','low'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className="text-[11px] px-3 py-1 rounded-full capitalize font-medium"
              style={{
                background: filter === f ? ROSE_LIGHT : 'transparent',
                color:      filter === f ? ROSE_TEXT : 'hsl(var(--muted-foreground))',
                border:     filter === f ? `1px solid ${ROSE}44` : '1px solid transparent',
              }}
            >
              {f === 'all' ? 'All' : PRIORITY_LABELS[f].label}
              {f !== 'all' && (
                <span className="ml-1 opacity-60">{items.filter(i => i.priority === f).length}</span>
              )}
            </button>
          ))}
        </div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {displayed.map(item => {
          ...
          return (
            <div
              key={item.id}
              className="rounded-2xl p-4 flex flex-col gap-2 bg-card border border-border hover:shadow-md transition-shadow"
            >
              ...
              <div className="flex items-center gap-2 mt-auto">
                <button
                  ...
                  style={{ background: `${pr.color}22`, color: pr.color, border: `1px solid ${pr.color}44` }}
                  title="Click to change priority"
                >
                  {pr.label}
                </button>
                <a
                  href={buildBuyLink(item.name, item.brand)}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1 text-[11px] font-medium px-2.5 py-1 rounded-full ml-auto"
